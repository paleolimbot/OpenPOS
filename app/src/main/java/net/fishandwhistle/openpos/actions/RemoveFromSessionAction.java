...
public class RemoveFromSessionAction extends ScannedItemAction {

    public static final String OPTION_SESSION_NAME = "session_name";

    private String sessionName;

    public RemoveFromSessionAction(JSONObject jsonObject) {
        super(jsonObject);
    ...
    public boolean doActionContent(Context context, ScannedItem item, ActionExecutor executor) throws ActionException {
        ...
        boolean result = manager.removeItem(sessionName, item);
        if(result) {
            return true;
        } else if(isQuiet()) {
            return false;
